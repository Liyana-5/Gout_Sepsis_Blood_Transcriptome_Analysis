...
#There are 14 females nd 13 males in the study and from each group 9 individuals are sampled resulting in a total of 27 observations 
#in the total sample min neutrophil count is 1.8 median is 7.6 max is 14.9 
...
#the result says the neutrophil count have significant difference between each group this may be due to immune responses due to the disease environment 
# the neutrophil count particularly varies between SA vs healthy group.

#identifying significant genes from the differential expressed genes between gout and healthy and sa and healthy
...
#creating expression table for all significant genes 
...
#PCA ON TOP 10 GENES OF HC vs GOUT and HC vs SEPSIS
...
# we can see the neutrophil counts effect the gene expressions in most groups in sepsis and some in gout accepting neutrophil null hypothesis.
# sex does not effect gene expression  accepting  null hypothesis
...
#since we see a significant difference in gene expression between SEPSIS and GOUT we reject the null hypothesis 
...
cat('\n____________________________________NULL HYPOTHESIS REJECTED BLOOD CAN BE USED TO DIFFERENCIAT BETWEEN G & SA_________________________________________________\n')
